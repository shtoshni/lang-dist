...
def get_tree_layout():
    ...
    ftype_dict = {"Romance": 'Computer Modern', "Germanic": 'Avant Garde',
                  "Slavic": 'Courier', "Hellenic": 'Verdana'}
...
def main():
    ...
    # Store tree
    out_dir = path.dirname(args.tree_file)
    tree_file = path.basename(args.tree_file)
    base_filename = path.splitext(tree_file)[0]
    output_file = path.join(out_dir, base_filename + ".pdf")
