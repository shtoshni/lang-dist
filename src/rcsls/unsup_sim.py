...
def calc_sim_mat(lang_to_emb, knn=10):
    sim_mat = np.zeros((len(lang_codes), len(lang_codes)))

    # Move a copy of embeddings to GPU as well
    cuda_embeddings = {}
    for lang in lang_codes:
        cuda_embeddings[lang] = torch.tensor(
            lang_to_emb[lang]['embeddings']).to(cuda0)

    # Index embeddings of all langs to allow for faster nearest search
    faiss_indices = {}
    for lang in lang_codes:
...
def main():
    ...
    threshold_str = str(int(args.threshold / 1000)) + "K"
    sim_mat_file = path.join(
        args.out_dir, "unsup_sim_mat_{}.npy".format(threshold_str))

    logging.info("Save similarity matrix at :%s" %sim_mat_file)
    np.save(sim_mat_file, sim_mat)
