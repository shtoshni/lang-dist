...
def compute_csls_neighbors_cpu(x_src, x_tgt, nn_distance_to_src_words):
    """Compute CSLS score
    x_src: B x d
    x_tgt: B' x d
    nn_distance_to_src_words: B'
    """
    similarities = x_src.mm(torch.transpose(x_tgt, 0, 1))#.cpu().numpy()
    similarities.mul_(2)
    similarities = similarities.cpu().numpy()

    similarities -= nn_distance_to_src_words[np.newaxis, :]
    nn_idx = np.argmax(similarities, axis=1, keepdims=True)
    return nn_idx
